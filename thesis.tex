...
\begin{abstract}
      In this thesis, we determine, to as large an extent as possible, the group structure of the $K$-valued points of an abelian variety when $K$ is either a real closed field or a $p$-adically closed field. Using the known structure of abelian varieties over $\bR$ and $\bQ_p$, we employ model-theoretic transfer principles to pass this information to abelian varieties over $K$.\hspace{2cm}
\end{abstract}
